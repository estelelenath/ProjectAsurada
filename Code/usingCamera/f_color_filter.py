...
cap = cv2.VideoCapture("D:\ProjectAsurada\ProjectAsurada\VideoSample\FrontCameraTestnn.mp4")
...
def wrapping(image):
    ...
    #one lane version
    source_point_left_upper = [w * 4.9 // 10, h * 0.58]
    source_point_right_upper = [w * 5.9 // 10, h * 0.58]
    source_point_left_bottom = [w * 0.21, h]
    source_point_right_bottom = [w* 0.85, h]

    destination_point_left_upper = [0, 0]
    destination_point_right_upper = [w-400, 0]
...
def color_filter(image):
    ...
    white_lower = np.array([np.round(  0 / 2), np.round(0.62 * 255), np.round(0.00 * 255)])
...
def roi(image):
    ...
    # 한 붓 그리기
    _shape = np.array(
        [[int(0.1*x), int(y)], [int(0.1*x), int(0.1*y)], [int(0.4*x), int(0.1*y)], [int(0.4*x), int(y)], [int(0.75*x), int(y)], [int(0.43*x), int(0.1*y)],[int(0.8*x), int(0.1*y)], [int(0.8*x), int(y)], [int(0.2*x), int(y)]])
...
while True:
    ...
    thickness = 30
    #cv2.circle(img, (int(w * 4.9 // 10), int(h * 0.58)), thickness, (255,0,0), -1)
    #cv2.circle(img, (int(w * 5.9 // 10), int(h * 0.58)), thickness, (255,0,0), -1)
    #cv2.circle(img, (int(w * 0.21), int(h)), thickness, (255,0,0), -1)
    ...
    #cv2.circle(wrapped_img, (int(0), int(0)), thickness, (255,0,0), -1)
    #cv2.circle(wrapped_img, (int(w-400), int(0)), thickness, (255,0,0), -1)
